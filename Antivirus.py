...
def get_system_info():
    ...
    info_text = '\n'.join([i[2:-2] for i in new])
    return info_text


def scanSlot(file_path):
    windefender = ctypes.windll.WindowsDefender

    result = windefender.WdIsFileExcluded(ctypes.c_wchar_p(file_path), False)
    if result == 0:
        detection_result = ctypes.c_int(0)
        scan_result = windefender.WdScanControl(
            ctypes.c_wchar_p(file_path),
            ctypes.byref(detection_result)
        )

        if scan_result == 0:
            if detection_result.value == 0:
                print(f"File: {file_path} - No threats detected")
            else:
                print(f"File: {file_path} - Threat detected")
        else:
            print(f"Error scanning file: {file_path}")
    else:
        print(f"File: {file_path} - Excluded from scanning")
...
# url´s
Report_issues = "https://github.com/ShairALiMughal/Python_Scanner/issues/new"
Submit_sample = "https://github.com/ShairALiMughal/Python_Scanner/discussions/8"
...
class Ui_MainWindow(object):
    def showNoDirectorySelectedPopup(self):
        msg = QtWidgets.QMessageBox()
        msg.setIcon(QtWidgets.QMessageBox.Warning)
        msg.setText("No directory selected.")
        msg.setWindowTitle("Warning")
        msg.exec_()
    def onSelectFileLocationClicked(self):
        directory = QtWidgets.QFileDialog.getExistingDirectory()
        # Check if a directory was selected
        if directory:
            scanSlot(directory)
        else:
            self.showNoDirectorySelectedPopup()

    def show_system_info_popup(self):
        system_info = get_system_info()
        messagebox.showinfo("System Requirements", system_info)

    def show_virus_info(self):
        virus_window = tk.Toplevel()
...
"image: url(:/res/SideBar/settings.svg);")
        ...
        self.showinfo = QtWidgets.QPushButton(self.HomeTab)
        self.showinfo.setGeometry(QtCore.QRect(5, (85+40), 121, 31))
        font = QtGui.QFont()
        font.setPointSize(11)
        self.showinfo.setFont(font)
        self.showinfo.setFlat(False)
        self.showinfo.setObjectName("show")
        self.showinfo.clicked.connect(self.show_system_info_popup)

        self.showvinfo = QtWidgets.QPushButton(self.HomeTab)
        self.showvinfo.setGeometry(QtCore.QRect(5, (85+80), 121, 31))
        font = QtGui.QFont()
        font.setPointSize(11)
        self.showvinfo.setFont(font)
        self.showvinfo.setFlat(False)
        self.showvinfo.setObjectName("showvinfo")
        self.showvinfo.clicked.connect(self.show_virus_info)
    ...
    def retranslateUi(self, MainWindow):
        ...
        MainWindow.setWindowTitle(_translate("MainWindow", f"-Malware Detections Using Ai- [v{VERSION}] [dev {DEV}]"))
        self.HomeTitle.setText(_translate("MainWindow", "Malware Detections Using Ai"))
        ...
        self.showinfo.setText(_translate("MainWindow", "System info"))
        self.showvinfo.setText(_translate("MainWindow", "Show Viruses"))
